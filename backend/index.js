  ...
  try {
    ...
    if (!user) {
      ...
      try {
        ...
        console.error('Error guardando historial de login (fallido):', e.message);
      }
    ...
    // Registrar inicio exitoso en historial
    try {
      const hist = new LoginHistory({
        _id: new mongoose.Types.ObjectId().toString(),
        user_id: user._id,
      ...
      console.error('Error guardando historial de login (exitoso):', e.message);
    }
...
// Endpoint para obtener historial de logins de un usuario ordenado del más reciente al más antiguo
app.get('/usuarios/:id/login-history', async (req, res) => {
  try {
    const userId = req.params.id;
    const history = await LoginHistory.find({ user_id: userId }).sort({ timestamp: -1 }).limit(100);
    res.json({ success: true, total: history.length, history });
  } catch (err) {
    res.status(500).json({ error: 'Error obteniendo historial de logins', details: err.message });
  }
});
